...
#' @rdname wilcoxauc
#' @export
wilcoxauc.Seurat <- function(X, group_by=NULL, assay='data', 
...
#' @rdname wilcoxauc
#' @export
wilcoxauc.SingleCellExperiment <- function(X, group_by=NULL, assay=NULL,
                                           groups_use=NULL, ...) {
    if (is.null(group_by)) {
    ...
    if (is.null(assay)) {
        ...
        standard_assays <- c('normcounts', 'logcounts', 'cpm', 'tpm',
                             'weights', 'counts')
